...
variables (L : language.{u}) (α β : Type v) {m m₁ m₂ n : ℕ}

inductive uniform_subterm (n : ℕ) : Type (max u v)
| uvar  {} : α → uniform_subterm
| var   {} : fin n → uniform_subterm
| function : ∀ {k}, L.fn k → (fin k → uniform_subterm) → uniform_subterm

@[reducible] def uniform_term := uniform_subterm L α 0

inductive uniform_subformula : ℕ → Type (max u v)
| verum    {n} : uniform_subformula n
| relation {n} : ∀ {p}, L.pr p → (fin p → uniform_subterm L α n) → uniform_subformula n
...
@[simp] def arity : uniform_subterm L ℕ n → ℕ
| &&x            := x + 1
| ##x            := 0
| (function f v) := ⨆ᶠ i, (v i).arity

@[simp] def to_subterm : ∀ t : uniform_subterm L ℕ n, t.arity ≤ m → subterm L m n
| &&x                   h := &⟨x, h⟩
| ##x                   h := #x 
| (@function L _ n k f v) h := subterm.function f (λ i, to_subterm (v i) (by { haveI: inhabited (fin k) := ⟨i⟩, simp at h, exact h i}))

section subst
variables {L α β n}

@[simp] def subst (t : uniform_subterm L α n) : uniform_subterm L α (n + 1) → uniform_subterm L α n
| &&x            := &&x
| ##x            := (var <* t) x
| (function f v) := function f (λ i, subst $ v i)

@[simp] def lift : uniform_subterm L α n → uniform_subterm L α (n + 1)
| &&x            := &&x
| ##x            := ##x.succ
| (function f v) := function f (λ i, (v i).lift)

@[simp] def map_metavar (f : α → β) : uniform_subterm L α n → uniform_subterm L β n
| &&x := &&(f x)
| ##x := ##x
| (function f v) := function f (λ i, (v i).map_metavar)

end subst

section semantics
variables (S : Structure L) {α m m₁ m₂ n} (me : α → S) (e : fin n → S)

@[simp] def val (me : α → S) (e : fin n → S) : uniform_subterm L α n → S
| &&x           := me x
| ##x           := e x
| (function f v) := S.fn f (λ i, (v i).val)

@[simp] lemma val_subst (u : uniform_subterm L α n) (me : α → S) (e : fin n → S) : ∀ (t : uniform_subterm L α (n + 1)),
  (subst u t).val S me e = t.val S me (e <* val S me e u)
| &&x            := by simp
| ##x            := by simp; refine fin.last_cases _ _ x; simp
| (function f v) := by simp; refine congr_arg _ (by ext i; exact val_subst (v i))

@[simp] lemma val_lift (x : S) (t : uniform_subterm L α n) :
