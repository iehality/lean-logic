...
inductive proof : preTheory L → subformula L 0 → Type u
| generalize   {T : preTheory L} : ∀ {p}, proof T.mlift p → proof T (∀'𝗡p)
| mdp          {T : preTheory L} : ∀ {p q}, proof T (p ⟶ q) → proof T p → proof T q
| by_axiom     {T : preTheory L} : ∀ {p}, p ∈ T → proof T p
| verum        {T : preTheory L} : proof T ⊤
| imply₁       {T : preTheory L} : ∀ {p q}, proof T (p ⟶ q ⟶ p)
| imply₂       {T : preTheory L} : ∀ {p q r}, proof T ((p ⟶ q ⟶ r) ⟶ (p ⟶ q) ⟶ p ⟶ r)
| contra       {T : preTheory L} : ∀ {p q}, proof T ((∼p ⟶ ∼q) ⟶ q ⟶ p)
| specialize   {T : preTheory L} : ∀ {p} {t}, proof T (∀'p ⟶ subst t p)
| dummy_univ   {T : preTheory L} : ∀ {p q}, proof T (∀'(dummy p ⟶ q) ⟶ p ⟶ ∀'q)
| non_empty    {T : preTheory L} : proof T (∃'⊤)

instance : has_Longarrow (formula L) := ⟨proof⟩

def provable (T : preTheory L) (p : formula L) : Prop := nonempty (T ⟹ p)
...
variables {T U : preTheory L}

lemma generalize {p} (h : T.mlift ⊢ p) : T ⊢ ∀'p.pull := by rcases h; exact ⟨h.generalize⟩

lemma generalize' {T : preTheory L} {p} (h : T ⊢ p) (hT : T = U.mlift) : U ⊢ ∀'p.pull :=
by rcases hT with rfl; exact generalize h

lemma gen {p : subformula L 1} (h : T.mlift ⊢ p.push) : T ⊢ ∀'p :=
...
@[elab_as_eliminator]
theorem rec_on {C : Π (T : preTheory L) (p : subformula L 0), T ⊢ p → Prop}
  {T : preTheory L} {p : formula L} (b : T ⊢ p)
  (generalize : ∀ {T : preTheory L} {p} (b : T.mlift ⊢ p), C T.mlift p b → C T (∀'p.pull) (generalize b))
  (mdp : ∀ {T : preTheory L} {p q} (b₁ : T ⊢ p ⟶ q) (b₂ : T ⊢ p), C T (p ⟶ q) b₁ → C T p b₂ → C T q (b₁ ⨀ b₂))
  (by_axiom : ∀ {T : preTheory L} {p} (h : p ∈ T), C T p (by_axiom h))
  (verum : ∀ {T : preTheory L}, C T ⊤ axiomatic_classical_logic'.provable_top)
  (imply₁ : ∀ {T : preTheory L} {p q}, C T (p ⟶ q ⟶ p) (axiomatic_classical_logic'.imply₁ p q))
  (imply₂ : ∀ {T : preTheory L} {p q r}, C T ((p ⟶ q ⟶ r) ⟶ (p ⟶ q) ⟶ p ⟶ r) (axiomatic_classical_logic'.imply₂ p q r))
  (contra : ∀ {T : preTheory L} {p q}, C T ((∼p ⟶ ∼q) ⟶ q ⟶ p) (axiomatic_classical_logic'.contraposition p q)) 
  (specialize : ∀ {T : preTheory L} {p} {t}, C T (∀'p ⟶ subst t p) (specialize T p t))
  (dummy_univ : ∀ {T : preTheory L} {p q}, C T (∀'(dummy p ⟶ q) ⟶ p ⟶ ∀'q) (dummy_univ T p q))
  (non_empty : ∀ {T : preTheory L}, C T (∃'⊤) (non_empty T)) :
  C T p b :=
begin
  rcases b with ⟨b⟩,
  induction b,
  case generalize : T p b IH { exact generalize ⟨b⟩ IH },
  case mdp : T p q b₁ b₂ IH₁ IH₂ { exact mdp ⟨b₁⟩ ⟨b₂⟩ IH₁ IH₂ },
  case by_axiom : T p hp { exact by_axiom hp },
  case verum : T { exact verum },
  case imply₁ : T p q { exact imply₁ },
  case imply₂ : T p q r { exact imply₂ },
  case contra : T p q { exact contra },
  case specialize : T p t { exact specialize },
  case dummy_univ : T p q { exact dummy_univ },
  case non_empty { exact non_empty }
end

noncomputable def provable.proof {T : preTheory L} {p : formula L} (b : T ⊢ p) : T ⟹ p := nonempty.some b

def weakening_aux {p} (h : T ⊢ p) : ∀ {U}, T ⊆ U → U ⊢ p :=
begin
  apply rec_on h,
  { intros T p b IH U hyp, refine generalize (IH $ set.image_subset _ hyp) },
  { intros T p q hyp_pq hyp_p IH₁ IH₂ U hyp, exact (IH₁ hyp) ⨀ (IH₂ hyp) },
  { intros T p hyp_p U hyp, exact by_axiom (hyp hyp_p) },
  { intros, simp },
  { intros, simp },
  { intros, simp },
  { intros, simp },
...
begin
  ...
  { rintros T p₁ p₂ b₁ b₂ IH₁ IH₂ U p rfl,
    show U ⊢ p ⟶ p₂, from (IH₁ U p rfl) ⨀₁ (IH₂ U p rfl) },
  { rintros T p hp U r rfl, rcases hp with (rfl | hp),
    { simp }, { have : U ⊢ p, from by_axiom hp, exact hyp_right this r } },
  { rintros T U p rfl, simp },
  { rintros T p q U r rfl, simp },
  { rintros T p q r U s rfl, simp },
  { rintros T p q U r rfl, simp },
  { rintros T p t U q rfl, refine hyp_right (specialize _ p t) _ },
  { rintros T p q U r rfl, refine hyp_right (dummy_univ _ p q) _ },
  { intros, simp }
end
...
lemma empty_axiom_generalize {p : formula L} (hp : ⬝⊢ p) : ⬝⊢ ∀'p.pull :=
by { have : preTheory.mlift ∅ ⊢ p, by simpa[preTheory.mlift] using hp, exact generalize this }

private lemma mlift_list_conjunction (P₀ : list (formula L)) : (∀ p, p ∈ P₀ → p ∈ T.mlift) →
  ∃ P : list (formula L), P.conjunction.mlift = P₀.conjunction ∧ (∀ p, p ∈ P → p ∈ T) :=
begin
  induction P₀ with p₀ P₀ IH,
  { intros _, refine ⟨[], by simp⟩ },
  { intros h,
    have : ∃ P : list (formula L), P.conjunction.mlift = P₀.conjunction ∧ (∀ p, p ∈ P → p ∈ T),
...
theorem finite_character_aux {T : preTheory L} {p} :
  T ⊢ p → ∃ P : list (formula L), (∀ p, p ∈ P → p ∈ T) ∧ ⬝⊢ P.conjunction ⟶ p := λ h,
begin
  apply rec_on h,
  { rintros T p b ⟨P₀, IH, IHb⟩,
    have : ∃ P : list (formula L), P.conjunction.mlift = P₀.conjunction ∧ ∀ p, p ∈ P → p ∈ T,
  ...
  { rintros T p q b₁ b₂ ⟨P₁, IH₁, IHb₁⟩ ⟨P₂, IH₂, IHb₂⟩,
    refine ⟨P₁ ++ P₂, _, _⟩,
    ...
    { have    : ⬝⊢ (P₁ ++ P₂).conjunction ⟶ P₁.conjunction, from list_conjunction_weakening (by simp),
      ...
      exact h₁ ⨀₁ h₂ } },
  { rintros T p hp, refine ⟨[p], by simp[hp], _⟩, simp, refine deduction.mp (by simp) },
  { rintros T, refine ⟨[], by simp, by simp[empty_axiom]⟩ },
  { rintros T p q, refine ⟨[], by simp, by simp[empty_axiom]⟩ },
  { rintros T p q r, refine ⟨[], by simp, by simp[empty_axiom]⟩ },
  { rintros T p q, refine ⟨[], by simp, by simp[empty_axiom]⟩ },
  { rintros T p t, refine ⟨[], by simp, by simp[empty_axiom]⟩ },
  { rintros T p q, refine ⟨[], by simp, by simp[empty_axiom]⟩ },
  { rintros T, refine ⟨[], by simp, by simp[empty_axiom]⟩ }
end

instance : has_finite_character (formula L) :=
finite_character_of_finite_provable (formula L) (λ T p, finite_character_aux)

lemma exists_of_subst (p : subformula L 1) (t) : T ⊢ subst t p ⟶ ∃'p :=
contrapose.mp (imply_of_equiv
  (show T ⊢ p.neg.fal ⟶ ∼subst t p, by simpa using specialize T (∼p) t)
  (iff_dn_refl_right $ ∀'∼p) (equiv_refl _))

lemma specialize' {T} (p : subformula L 1) : T ⊢ ∀' 𝗟 p ⟶ 𝗠 p :=
by { have : T ⊢ ∀' 𝗟 p ⟶ subst &0 p.mlift, from specialize T p.mlift &0, simpa using this }

lemma use {p : subformula L 1} (t) (h : T ⊢ subst t p) : T ⊢ ∃'p :=
...
@equiv_forall_of_equiv _ T (𝗡 p₁) (𝗡 p₂) (by simpa using hp)
...
@equiv_exists_of_equiv _ T (𝗡 p₁) (𝗡 p₂) (by simpa using hp)

lemma univ_imply_dummy (p : subformula L 1) (q : subformula L 0) :
  T ⊢ ∀'(p ⟶ 𝗗 q) ⟶ ∃'p ⟶ q :=
begin
...
lemma exists_intro (p : subformula L 1) (q : subformula L 0)
  (h : 𝗟'T ⊢ 𝗠 p ⟶ 𝗟 q) : T ⊢ ∃'p ⟶ q :=
...
@[simp] lemma forall_dummy (p : formula L) : T ⊢ ∀'𝗗 p ⟷ p :=
begin
...
lemma neg_univ_closure_pnf {n} (p : subformula L n) : T ⊢ ∼∀'*p ⟷ ∃'*∼p :=
begin
  induction n with n IH generalizing T, { simp },
...
lemma neg_exists_closure_pnf {n} (p : subformula L n) : T ⊢ ∼∃'*p ⟷ ∀'*∼p :=
begin
  induction n with n IH generalizing T, { simp },
  { simp[forall_comm, subformula.exists_comm],
...
lemma specialize_foralls (p : subformula L n) (w : fin n → subterm L 0) : T ⊢ ∀'*p ⟶ substs w p :=
begin
...
lemma foralls_substs {p : subformula L n} (h : T ⊢ ∀'*p) (w) : T ⊢ substs w p :=
specialize_foralls p w ⨀ h

@[simp] lemma exists_dn (p : subformula L n) : T ⊢ ∃'*∼∼p ⟷ ∃'*p :=
begin
  induction n with n IH generalizing T; simp[subformula.exists_comm],
  refine equiv_exists_of_equiv (by simpa using IH (𝗠 p))
end

@[simp] lemma neg_univ_closure_neg (p : subformula L n) : T ⊢ ∼∀'*(∼p) ⟷ ∃'*p :=
begin
  have : T ⊢ ∼∀'*(∼p) ⟷ ∃'*(∼∼p), from neg_univ_closure_pnf (∼p),
  refine equiv_trans this (by simp)
end

@[simp] lemma exists_substs (p : subformula L n) (w : fin n → subterm L 0) : T ⊢ substs w p ⟶ ∃'*p :=
begin
  have : T ⊢ ∼∼substs w p ⟶ ∼∀'* (∼p), by simpa using contrapose.mpr (specialize_foralls (∼p) w),
  refine imply_of_equiv this (by simp) (by simp)
end

lemma exists_of_substs {p : subformula L n} (w) (h : T ⊢ substs w p) : T ⊢ ∃'*p :=
exists_substs p w ⨀ h
...
def eq_axiom_schema_funext [L.has_equal] {k} (f : L.fn k) : subformula L 0 :=
∀'*((⋀ i, #(fin.cast_add k i) =' #(fin.nat_add k i)) ⟶
  (function f (var ∘ fin.cast_add k) =' function f (var ∘ fin.nat_add k)) : subformula L (k + k))

def eq_axiom_schema_relext [L.has_equal] {k} (r : L.pr k) : subformula L 0 :=
∀'*((⋀ i : fin k, #(fin.cast_add k i) =' #(fin.nat_add k i)) ⟶
  (relation r (var ∘ fin.cast_add k) ⟷ relation r (var ∘ fin.nat_add k)))

variables {L} [language.has_equal L]

inductive Eq : preTheory L
| eq_refl : Eq (∀'(#0 =' #0))
| eq_symm : Eq (∀' ∀'((#0 =' #1) ⟶ (#1 =' #0)))
...
@[simp] lemma mlift_Eq : (Eq : preTheory L).mlift = Eq :=
begin
...
--def eq_extend {T : preTheory L} [extend Eq T] : extend Eq 𝗟'T :=
