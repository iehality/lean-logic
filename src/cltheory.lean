...
def herbrand (n : ℕ) : setoid (term L) := ⟨λ t₁ t₂, T^n ⊢̇ t₁ =̇ t₂, vecterm_equiv_equivalence (T^n)⟩
...
@[simp] lemma equivs_equals : ∀ {n} (v₁ v₂ : dvector (term L) (n+1)),
  (@setoid.vec_r _ (herbrand T i) _) v₁ v₂ ↔ (T^i ⊢̇ v₁.to_vecterm ≡̇ v₂.to_vecterm)
...
| (n+1) p v := fopl.Herbrand.lift_on_vec v (λ u : dvector (term L) (n+1), T^i ⊢̇ formula.app p (u.to_vecterm))
...
protected theorem provable_iff {t₁ t₂} : T^i ⊢̇ t₁ =̇ t₂ ↔ (⟦t₁⟧ᴴ : Herbrand T i) = ⟦t₂⟧ᴴ := by simp[of_eq_of]
protected theorem provable_iff0 {t₁ t₂} : T ⊢̇ t₁ =̇ t₂ ↔ (⟦t₁⟧ᴴ : Herbrand T 0) = ⟦t₂⟧ᴴ := by simp[of_eq_of]
...
λ t₁ t₂ hyp, by { simp[Herbrand.of_eq_of, -provable.iff] at*,
    ...
    have lmm : T^i ⊢̇ t₁ =̇ t₂, exactI provable.inclusion hyp (ordered_inclusion _),
    refine provable.ppc_prove_rew i lmm ₛ[t] }

variables [proper 0 T]

def subst_sf_H (i) : Herbrand T 0 → Herbrand T (i+1) → Herbrand T i :=
...
notation h₁ ` ⊳ᴴ `h₂ :90  := subst_sf_H _ h₁ h₂
...
by { induction h₁ using fopl.Herbrand.ind_on, induction h₂ using fopl.Herbrand.ind_on,
     show ⟦h₁⟧ᴴ ⊳ᴴ[i] function₁ T (i + 1) f ⟦h₂⟧ᴴ = function₁ T i f (⟦h₁⟧ᴴ ⊳ᴴ ⟦h₂⟧ᴴ),
...
@[simp] lemma subst_sf_H_function₂
  ...
  h₁ ⊳ᴴ[i] (function₂ _ _ f h₂ h₃) = function₂ _ _ f (h₁ ⊳ᴴ h₂) (h₁ ⊳ᴴ h₃) :=
...
protected def lift_on {φ} (d : Lindenbaum T i) (f : formula L → φ)
  (h : ∀ p q : formula L, T^i ⊢̇ p ↔̇ q → f p = f q) : φ :=
quotient.lift_on' d f h

@[simp]
protected lemma lift_on_eq {φ} (p : formula L) (f : formula L → φ)
  (h : ∀ p q, T^i ⊢̇ p ↔̇ q → f p = f q) : fopl.Lindenbaum.lift_on ⟦p⟧ᴸ f h = f p := rfl

@[elab_as_eliminator, reducible, simp]
protected def lift_on₂ {φ} (d₁ d₂ : Lindenbaum T i) (f : formula L → formula L → φ)
  (h : ∀ p₁ p₂ q₁ q₂, T^i ⊢̇ p₁ ↔̇ q₁ → T^i ⊢̇ p₂ ↔̇ q₂ → f p₁ p₂ = f q₁ q₂) : φ :=
quotient.lift_on₂' d₁ d₂ f h

@[simp]
protected lemma lift_on₂_eq {φ} (p q : formula L) (f : formula L → formula L → φ)
  (h : ∀ p₁ p₂ q₁ q₂, T^i ⊢̇ p₁ ↔̇ q₁ → T^i ⊢̇ p₂ ↔̇ q₂ → f p₁ p₂ = f q₁ q₂) :
  fopl.Lindenbaum.lift_on₂ ⟦p⟧ᴸ.T ⟦q⟧ᴸ.T f h = f p q := rfl

protected lemma of_eq_of {p q : formula L} : (⟦p⟧ᴸ : Lindenbaum T i) = ⟦q⟧ᴸ ↔ T^i ⊢̇ p ↔̇ q :=
by simp[formula.quo, formula.equiv, quotient.eq']

instance : has_le (Lindenbaum T i) :=
⟨λ p₁ p₂, Lindenbaum.lift_on₂ p₁ p₂ (λ p₁ p₂, T^i ⊢̇ p₁ →̇ p₂) $
...
--notation h₁ ` p⟪` f `⟫² ` h₂ := predicate₂ _ f h₁ h₂
...
λ t₁ t₂ u₁ u₂ eqn₁ eqn₂, by {
  ...
  have lmm₁ : (T^i)+{t₁ =̇ t₂} ⊢̇ u₁ =̇ t₁, simp [provable.e2.MP eqn₁],
  have lmm₂ : (T^i)+{t₁ =̇ t₂} ⊢̇ t₁ =̇ t₂, simp,
  have lmm₃ : (T^i)+{t₁ =̇ t₂} ⊢̇ t₂ =̇ u₂, simp [eqn₂],
  refine (lmm₁.eq_trans lmm₂).eq_trans lmm₃,
  have lmm₁ : (T^i)+{u₁ =̇ u₂} ⊢̇ t₁ =̇ u₁, simp [eqn₁],
  have lmm₂ : (T^i)+{u₁ =̇ u₂} ⊢̇ u₁ =̇ u₂, simp,
  have lmm₃ : (T^i)+{u₁ =̇ u₂} ⊢̇ u₂ =̇ t₂, simp [provable.e2.MP eqn₂],
  refine (lmm₁.eq_trans lmm₂).eq_trans lmm₃ }
infix ` ∥ `:50 := equal

def Box : Lindenbaum T i → Prop := λ p, Lindenbaum.lift_on p (λ p, T^i ⊢̇ p) $
...
by { induction l using fopl.Lindenbaum.ind_on,
     ...
     refine provable.deduction.mp (provable.deduction.mp _),
     have lmm₁ : (T^i)+{(¬̇l →̇ m) ⩑ (¬̇l →̇ n)}+{¬̇l} ⊢̇ (¬̇l →̇ m) ⩑ (¬̇l →̇ n), simp[-provable.and],
     have lmm₂ : (T^i)+{(¬̇l →̇ m) ⩑ (¬̇l →̇ n)}+{¬̇l} ⊢̇ ¬̇l, simp,
...
by { induction l using fopl.Lindenbaum.ind_on, induction m using fopl.Lindenbaum.ind_on,
     ...
     refine deduction.mp (explosion (show (T^i)+{l ⩑ m ⩑ (l ⩑ ¬̇m)} ⊢̇ m, by simp[axiom_and]) (by simp[axiom_and])) }
...
by { induction l using fopl.Lindenbaum.ind_on, simp[has_le.le, has_inf.inf, has_compl.compl, has_bot.bot],
     have : (T^i)+{l ⩑ ¬̇l} ⊢̇ l ⩑ ¬̇l, simp[-provable.and], simp at this,
...
@[simp] def subst_sf_L_aux (t : term L) (j) :
  Lindenbaum T (j+i+1) → Lindenbaum T (j+i) :=
λ p, Lindenbaum.lift_on p (λ p, (⟦p.rew (ₛ[t]^i)⟧ᴸ : Lindenbaum T (j+i))) $
λ p₁ p₂ hyp, by { simp[Lindenbaum.of_eq_of, -provable.iff] at*,
    have lmm : T^(j+i) ⊢̇ p₁ ↔̇ p₂, from provable.inclusion hyp (proper_sf_inclusion T (show j+i ≤ j + i + 1, by omega)),
    have := provable.ppc_prove_rew (j+i) lmm ₛ[t], }

#check @provable.ppc_prove_rew

def subst_sf_L {j} : Herbrand T j → Lindenbaum T (j+i+1) → Lindenbaum T i :=
λ t l, Herbrand.lift_on t (λ t, subst_sf_L_aux t j l) $
...
lemma fal_le_subst_sf_L (l : Lindenbaum T (i+1)) (h) : ∏l ≤ (h ⊳ l) :=
begin
...
lemma subst_sf_L_le_ex (l : Lindenbaum (T^1)) (h) : h ⊳[0] l ≤ ∐l :=
begin
  induction l using fopl.Lindenbaum.ind_on, 
  induction h using fopl.Herbrand.ind_on, 
...
@[simp] lemma subst_sf_L_and (h : Herbrand T 0) (n) (l m : Lindenbaum T (i+1)) :
  h ⊳[n] (l ⊓ m) = h ⊳[n] l ⊓ h ⊳[n] m :=
by { induction l using fopl.Lindenbaum.ind_on, induction m using fopl.Lindenbaum.ind_on, 
     induction h using fopl.Herbrand.ind_on,
     simp[has_inf.inf, has_le.le, subst_sf_L, Lindenbaum.of_eq_of] }

@[simp] lemma subst_sf_L_or (h : Herbrand T 0) (n) (l m : Lindenbaum T (i+1)) :
  h ⊳[n] (l ⊔ m) = h ⊳[n] l ⊔ h ⊳[n] m :=
by { induction l using fopl.Lindenbaum.ind_on, induction m using fopl.Lindenbaum.ind_on, 
     induction h using fopl.Herbrand.ind_on,
     simp[has_sup.sup, has_le.le, subst_sf_L, Lindenbaum.of_eq_of] }

@[simp] lemma subst_sf_L_neg (h : Herbrand T 0) (n) (l : Lindenbaum T (i+1)) :
  h ⊳[n] lᶜ = (h ⊳[n] l)ᶜ :=
by { induction l using fopl.Lindenbaum.ind_on,
     induction h using fopl.Herbrand.ind_on,
     simp[has_compl.compl, has_le.le, subst_sf_L, Lindenbaum.of_eq_of] }

@[simp] lemma subst_sf_L_equal (h₁ : Herbrand T) (n) (h₂ h₃ : Herbrand (T^(n+1))) :
...
@[simp] lemma subst_sf_L_fal (h : Herbrand T 0) (n) (l : Lindenbaum (T^(n+2))) :
  h ⊳[n] ∏l = ∏(h ⊳[n+1] l) :=
by { induction l using fopl.Lindenbaum.ind_on,
     induction h using fopl.Herbrand.ind_on,
     simp[fal, has_le.le, subst_sf_L, Lindenbaum.of_eq_of, has_pow.pow, rewriting_sf_itr] }

@[simp] lemma subst_sf_L_ex (h : Herbrand T 0) (n) (l : Lindenbaum (T^(n+2))) :
  h ⊳[n] ∐l = ∐(h ⊳[n+1] l) :=
by { induction l using fopl.Lindenbaum.ind_on,
     induction h using fopl.Herbrand.ind_on,
     simp[ex, has_le.le, subst_sf_L, Lindenbaum.of_eq_of, has_pow.pow, rewriting_sf_itr] }

lemma subst_sf_L_sentence (h : Herbrand T 0) {n} {p : formula L} (a : sentence p) :
  h ⊳[n] (⟦p⟧ᴸ : Lindenbaum T (i+1)) = ⟦p⟧ᴸ :=
...
begin
  ...
  { refine (deduction.mp $ GE $ contrapose.mp _), rw [←sf_dsb], simp,
    have lmm₁ : ⇑(T ^ i)+{¬̇(∀̇l).sf →̇ k.sf} ⊢̇ ¬̇k.sf →̇ (∀̇l).sf, { apply contrapose.mp, simp },
    have lmm₂ : ⇑(T ^ i)+{¬̇(∀̇l).sf →̇ k.sf} ⊢̇ (∀̇l).sf →̇ l,
    { suffices : ⇑(T ^ i)+{¬̇(∀̇l).sf →̇ k.sf} ⊢̇ (∀̇l).sf →̇ (l.rew $ shift^1).rew ₛ[#0],
      { simp* at* },
      exact provable.q1 },
  ...
  { refine (deduction.mp $ contrapose.mp $ deduction.mp _), simp,
    ...
    show ⇑(T ^ i)+{(∀̇(¬̇l →̇ k.sf)).sf} ⊢̇ ¬̇k.sf →̇ l,
    have : ⇑(T ^ i)+{(∀̇(¬̇l →̇ k.sf)).sf} ⊢̇ ¬̇l →̇ k.sf,
    { have : ⇑(T ^ i)+{(∀̇(¬̇l →̇ k.sf)).sf} ⊢̇ (∀̇(¬̇l →̇ k.sf)).sf, { simp },
...
begin
  ...
  { refine (deduction.mp $ GE _), rw [←sf_dsb], simp[axiom_and],
    have : ⇑(T ^ i)+{(∀̇l).sf}+{k.sf} ⊢̇ (∀̇l).sf, simp,
    have := this.fal_subst #0, simp* at* },
  { refine deduction.mp _, simp,
     split,
    { refine GE _, simp[←sf_dsb],
      have : ⇑(T ^ i)+{(∀̇(l ⩑ k.sf)).sf} ⊢̇ (∀̇(l ⩑ k.sf)).sf, simp,
      have := this.fal_subst #0, simp* at* },
    { have : (T ^ i)+{∀̇(l ⩑ k.sf)} ⊢̇ ∀̇(l ⩑ k.sf), simp,
      have := this.fal_subst #0, simp* at * } }
end
...
theorem provable_top_iff {p} : T^i ⊢̇ p ↔ (⟦p⟧ᴸ : Lindenbaum T i) = ⊤ := by simp[has_top.top, Lindenbaum.of_eq_of]

protected theorem provable_iff {p q} : T^i ⊢̇ p ↔̇ q ↔ (⟦p⟧ᴸ : Lindenbaum T i) = ⟦q⟧ᴸ :=
by simp[Lindenbaum.of_eq_of]

protected theorem provable_iff0 {p q} : T ⊢̇ p ↔̇ q ↔ (⟦p⟧ᴸ : Lindenbaum T 0) = ⟦q⟧ᴸ :=
by simp[Lindenbaum.of_eq_of]

theorem provable_imp_iff {p q} : T^i ⊢̇ p →̇ q ↔ (⟦p⟧ᴸ : Lindenbaum T i) ≤ ⟦q⟧ᴸ := by simp[has_le.le]
...
@[simp] theorem provable_imp_eq {p q} : (⟦p →̇ q⟧ᴸ : Lindenbaum T i) = ⟦p⟧ᴸᶜ ⊔ ⟦q⟧ᴸ := by {
  ...
  { simp[Lindenbaum.of_eq_of, -provable_or_eq, formula.or], refine ⟨deduction.mp (by simp), deduction.mp _⟩,
    have : (T^i)+{¬̇¬̇p →̇ q} ⊢̇ ¬̇¬̇p →̇ q, simp[-dn1_iff], simp* at* },
  simp[this] }
...
@[simp] theorem provable_fal_eq {p} : (⟦∀̇p⟧ᴸ : Lindenbaum T i) = ∏⟦p⟧ᴸ := rfl
@[simp] theorem provable_ex_eq {p} : (⟦∃̇p⟧ᴸ : Lindenbaum T i) = ∐⟦p⟧ᴸ := rfl
...
theorem provable_neg_iff {p} : T^i ⊢̇ ¬̇p ↔ (⟦p⟧ᴸ : Lindenbaum T i) = ⊥ :=
