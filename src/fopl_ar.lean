...
class has_eq_symbol (L : language) := (eq : L.pr 2)
...
instance [has_eq_symbol L] {β} : has_eq (term L β) (formula L β) := ⟨λ t u, formula.app has_eq_symbol.eq ‹t, u›⟩
...
by { induction k with k IH; simp[*, has_shift.shifts_succ, shift_eq, nested_rew, ←nat.add_one, add_assoc], refl }

lemma pow_eq_rew_var (t : term L β) (k) : t ↟ k = t.rew_var (λ x, ⟨x + k, by simp⟩) :=
by simp[pow_eq]
...
by induction k with k IH; simp[*, has_shift.shifts_succ, ←nat.add_one, add_assoc]

@[simp] lemma app_pow {n} (f : L.fn n) (v : finitary' (term L) n β) (k : ℕ) :
  (❨f❩ v) ↟ k = ❨f❩ (v ↟ k) :=
by {induction k with k IH; simp[*, has_shift.shifts_succ], funext i, simp }
...
@[simp] lemma pow_inj {β} {t u : term L β} {k : ℕ} : t ↟ k = u ↟ k ↔ t = u :=
by { simp[pow_eq_rew_var], refine rew_var_inj_of_inj (λ x y, by { simp, exact fin.ext }) }
...
@[simp] lemma eq_rew [c : has_eq_symbol L] (t u : term L β) (s : finitary (term L γ) β) :
  (t ≃ u : formula L β).rew s = (t.rew s ≃ u.rew s) :=
by unfold has_eq.eq; simp
...
by induction k; simp[*, has_shift.shifts_succ]

@[simp] lemma unary_pow (p : L.pr 1) (k : ℕ) (t : term L β) : (❴p❵ ‹t›) ↟ k = ❴p❵ ‹t ↟ k› :=
by induction k with k IH; simp[*, has_shift.shifts_succ]

@[simp] lemma binary_pow (p : L.pr 2) (k : ℕ) (t₁ t₂ : term L β) :
  (❴p❵ ‹t₁, t₂›) ↟ k = ❴p❵ ‹t₁ ↟ k, t₂ ↟ k› :=
by induction k with k IH; simp[*, has_shift.shifts_succ]

@[simp] lemma eq_pow [c : has_eq_symbol L] (t u : term L β) (k : ℕ) :
  (t ≃ u : formula L β) ↟ k = (t ↟ k ≃ u ↟ k) :=
by unfold has_eq.eq; induction k with k IH; simp*
...
lemma fal_shift_discard (p : formula L (β + 1)) :
  ⤉(𝚷 p) = 𝚷 (p.rew (rewsf (λ x, #x.succ))) :=
by simp[shift_eq]
...
lemma ex_shift_discard (p : formula L (β + 1)) :
  ⤉(𝚺 p) = 𝚺 (p.rew (rewsf (λ x, #x.succ))) :=
by simp[shift_eq]
...
by { induction k with k IH generalizong s, { simp },
     { simp[has_shift.shifts_succ, ←nat.add_one, nested_rew, IH, shift_eq],
...
@[simp] lemma pow_inj {p q : formula L β} {k : ℕ} : p ↟ k = q ↟ k ↔ p = q :=
by { simp[pow_eq_rew_var], refine rew_var_inj_of_inj (λ x y, by { simp, exact fin.ext }) }
...
@[simp] lemma is_open.eq [c : has_eq_symbol L] (t u : term L β) : (t ≃ u : formula L β).is_open = tt := rfl

@[simp] lemma is_open.le [c : has_le_symbol L] (t u : term L β) : (t ≼ u : formula L β).is_open = tt := rfl

@[simp] lemma is_open.mem [c : has_mem_symbol L] (t u : term L β) : (t ∊ u : formula L β).is_open = tt := rfl
...
@[reducible] def Theory (L : language.{u}) := Π β, theory L β
...
instance : has_shift (theory L) := ⟨λ β T, (λ p : formula L β, ⤉p) '' T⟩

class proper_theory (T : Theory L) :=
(proper : ∀ {β} {p : formula L β} (s), p ∈ T β → p.rew s ∈ T β)

end theory

def eq_axiom4 [c:has_eq_symbol L] {n} (f : L.fn n) : formula L 0 :=
  𝚷* ((⋀ i : fin n, (#⟨i, nat.lt_add_left _ n n i.property⟩ ≃ (#⟨n + i, by simp⟩ : term L (n + n)))) ⟶
    ((term.app f (λ i, #⟨i, nat.lt_add_left _ n n i.property⟩)) ≃
      (term.app f (λ i, #⟨n + i, by simp⟩)) : formula L (n + n)))

def eq_axiom5 [c : has_eq_symbol L] {n} (r : L.pr n) : formula L 0 :=
  𝚷* ((⋀ i : fin n, (#⟨i, nat.lt_add_left _ n n i.property⟩ ≃ (#⟨n + i, by simp⟩ : term L (n + n)))) ⟶
    ((formula.app r (λ i, #⟨i, nat.lt_add_left _ n n i.property⟩)) ⟶
      (formula.app r (λ i, #⟨n + i, by simp⟩)) : formula L (n + n)))

inductive provable (T : Theory L) : Π {β}, formula L β → Type u
| GE : ∀ {β} {p : formula L (β + 1)},
    provable p → provable (𝚷 p)
| MP : ∀ {β} {p q : formula L β},
    provable (p ⟶ q) → provable p → provable q
| AX : ∀ {β} {p : formula L β},
    p ∈ T β → provable p
| p0 : ∀ {β},
    provable (⊤ : formula L β)
| p1 : ∀ {β} {p q : formula L β},
    provable (p ⟶ q ⟶ p)
| p2 : ∀ {β} {p q r : formula L β},
    provable ((p ⟶ q ⟶ r) ⟶ (p ⟶ q) ⟶ p ⟶ r)
| p3 : ∀ {β} {p q : formula L β},
    provable ((⁻p ⟶ ⁻q) ⟶ q ⟶ p)
| q1 : ∀ {β} {p : formula L (β + 1)} {t : term L β},
    provable (𝚷 p ⟶ p.rew (t ::ᶠ ι))
| q2 : ∀ {β} {p q : formula L (β + 1)},
    provable (𝚷 (p ⟶ q) ⟶ 𝚷 p ⟶ 𝚷 q)
| q3 : ∀ {β} {p : formula L β},
    provable (p ⟶ 𝚷 (⤉p))
